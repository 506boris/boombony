    ...
    public static void drawSteps(List<Coo> currentSteps, int maxStepCount, WorldTxLibrary txLibrary) {
        ...
        for (int i = currentSteps.size() - 1; i >= 0; i--) {
            ...
            if (currentSteps.size() - (i + 1) <= maxStepCount) {
                WorldTiledMap.cursorLayer.setCell(currentCoo.x, currentCoo.y, new Step(txLibrary.txRegion("maps/step.png"), currentCoo).getCell());
            } else {
                WorldTiledMap.cursorLayer.setCell(currentCoo.x, currentCoo.y, new Step(txLibrary.txRegion("maps/step-out.png"), currentCoo).getCell());
            }
        }
    }
